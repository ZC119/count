...
def count(settings, file_list, mode, shuffle):
        ...
        for image in images:
            ...
            with open(label_path) as flabel:
                for line in flabel:
                    ...
                    bbox_sample.append(bbox[0]/float(im_width))
                    bbox_sample.append(bbox[1]/float(im_height))
                    bbox_sample.append(bbox[2]/float(im_width))
                    bbox_sample.append(bbox[3]/float(im_height))
...
def train(settings, file_list, shuffle=True):
    ...
    return count(settings, file_list, 'train', shuffle)


def test(settings, file_list):
    file_list = os.path.join(settings.data_dir, file_list)
    return count(settings, file_list, 'test', False)
